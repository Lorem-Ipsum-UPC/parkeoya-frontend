...
const generateReviews = () => {
  const names = ["Juan Pérez", "María García", "Carlos López", "Ana Martínez", "Luis Rodríguez", "Sofia Torres"]
  const comments = [
  ...
  return Array.from({ length: 25 }, (_, i) => {
    ...
    return {
      ...
      response: hasResponse
        ...
        : null,
      responseDate: hasResponse ? new Date(Date.now() - Math.random() * 20 * 86400000).toISOString() : null,
    ...
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="pt-6">
            <div>
              ...
              <p className="text-3xl font-bold mt-1">{stats.responseRate.toFixed(0)}%</p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            <div>
              ...
              <p className="text-3xl font-bold mt-1">{reviews.filter((r) => !r.hasResponse).length}</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="pending">Pendientes ({reviews.filter((r) => !r.hasResponse).length})</TabsTrigger>
          <TabsTrigger value="responded">Respondidas ({reviews.filter((r) => r.hasResponse).length})</TabsTrigger>
        </TabsList>
                    ...
                    <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-100 dark:border-blue-900">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          <p className="text-sm text-blue-800 dark:text-blue-200 leading-relaxed">{review.response}</p>
                        </div>
                      </div>
                    </div>
                  )}
