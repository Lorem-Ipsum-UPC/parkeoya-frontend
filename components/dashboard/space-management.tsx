...
export function SpaceManagement() {
  ...
  const filteredSpaces = spaces.filter((space) => space.number.toLowerCase().includes(searchQuery.toLowerCase()))
              ...
              return (
                <button
                  ...
                  className={`aspect-square rounded-lg border-2 transition-all hover:scale-105 ${
                    selectedSpace?.id === space.id ? "border-blue-600 ring-2 ring-blue-600/20" : "border-transparent"
                  } ${config.bgColor}`}
                  title={`${space.number} - ${config.label}`}
                >
                  <div className="flex flex-col items-center justify-center h-full p-1">
                    <Icon className={`w-4 h-4 mb-1 ${config.textColor}`} />
                    <span className={`text-xs font-medium ${config.textColor}`}>{space.number}</span>
                  </div>
                </button>
              )
            })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div>
                  ...
                  <Badge className={statusConfig[selectedSpace.status as keyof typeof statusConfig].bgColor}>
                    <span className={statusConfig[selectedSpace.status as keyof typeof statusConfig].textColor}>
                ...
                <div>
                  <p className="text-sm text-muted-foreground mb-1">ID del Sensor</p>
                  <p className="font-mono text-sm">{selectedSpace.sensorId}</p>
                </div>

                <div>
                  <p className="text-sm text-muted-foreground mb-1">Tipo de Espacio</p>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm">{new Date(selectedSpace.lastUpdate).toLocaleString("es-ES")}</p>
                </div>

                <div>
                  <p className="text-sm text-muted-foreground mb-1">Conexión del Sensor</p>
                  <div className="flex items-center gap-2">
